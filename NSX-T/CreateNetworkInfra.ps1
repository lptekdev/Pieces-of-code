...
if ($edge_cluster.status -ne 200){
    ...
    if ($dhcp.status -notlike "CREATED"){
        ...
        $segment = NewSegment -transport_zone $credentials.transport_zone -segment_name $segment_name -tier1_gw_name $tier1_gw_name -network $network -gateway $gateway -dhcp_ranges $dhcp_ranges -dhcp_path $dhcp.dhcp_path -dhcp_server_ip $dhcp.server_address
        $segment
    }
